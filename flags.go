...
func NewFlags() (*flags, error) {
	...
	session := flag.Bool("session", false, "DEFAULT false - Prevent typeworm from stopping after recording to or playing back from a single file. If  If the file name contains %d and MODE is record, typeworm will first try to record to the file with %d equal to the OFFSET flag. After recording the offset value will be autoincremented to record to the next file in the sequence.")
	stopKey := flag.String("stop-key", "ESC", "DEFAULT ESC - Label of the key used to stop recording and exit typeworm. The chosen label must be mapped to a known keycode, and won't be recorded.")
